...
class ProjectTaskType(models.Model):
    ...
    def unlink_wizard(self, stage_view=False):
        ...
        readgroup = self.with_context(active_test=False).env['project.task'].read_group([('stage_id', 'in', self.ids)],
...
class Task(models.Model):
    ...
    subtask_planned_hours = fields.Float("Sub-tasks Planned Hours", compute='_compute_subtask_planned_hours',
                                         help="Sum of the time planned of all the sub-tasks linked to this task. Usually less or equal to the initially time planned of this task.")
    ...
    # # Tawaref
    #
    # deal_manager = fields.Char(string='deal')
    ...
    def _compute_recurring_count(self):
        ...
        count = self.env['project.task'].read_group([('recurrence_id', 'in', recurring_tasks.recurrence_id.ids)],
                                                    ['id'], 'recurrence_id')
    ...
    def _compute_ribbon_message(self):
        for task in self:
            ...
            if will_write_email and will_write_phone:
                task.ribbon_message = _(
                    'By saving this change, the customer email and phone number will also be updated.')
        ...
        for task in self.filtered(lambda t: t.project_id.privacy_visibility != 'portal'):
            ...
            if portal_users:
                ...
                raise ValidationError(
                    _("The project visibility setting doesn't allow portal users to see the project's tasks. (%s)",
                      user_names))
    ...
    def message_subscribe(self, partner_ids=None, channel_ids=None, subtype_ids=None):
        ...
        """
        res = super(Task, self).message_subscribe(partner_ids=partner_ids, channel_ids=channel_ids,
                                                  subtype_ids=subtype_ids)
        if partner_ids:
    ...
    def write(self, vals):
        ...
        if recurrence_update != 'this':
            ...
            if recurrence_update == 'subsequent':
                for task in self:
                    recurrence_domain = OR([recurrence_domain, ['&', ('recurrence_id', '=', task.recurrence_id.id),
    ...
    def message_new(self, msg, custom_values=None):
        ...
        customer_ids = [p.id for p in self.env['mail.thread']._mail_find_partner_from_emails(
            tools.email_split(defaults['email_from']), records=task) if p]
    ...
    def rating_apply(self, rate, token=None, feedback=None, subtype_xmlid=None):
        return super(Task, self).rating_apply(rate, token=token, feedback=feedback,
